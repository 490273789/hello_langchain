...
async def summarization_memory_demo() -> None:
    ...
    for msg in messages:
        if msg["type"] == "human":
            await history.aadd_messages([HumanMessage(content=msg["content"])])
        else:
            await history.aadd_messages([AIMessage(content=msg["content"])])

    all_messages = await history.aget_messages()
    ...
    if total_tokens >= max_tokens:
        ...
        await history.aadd_messages(recent_messages)
